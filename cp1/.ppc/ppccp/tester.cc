...
static float verify_gvfa(const input &input, const float *result, int iter) {
    ...
    for (int k = 0; k < iter; k++) {
        std::vector<double> x(input.ny);
        for (int j = 0; j < input.ny; j++)
            x[j] = rng.get_double_normal();

        std::vector<double> ATx(input.nx);
        for (int i = 0; i < input.nx; i++) {
            double temp = 0.0;
            for (int j = 0; j < input.ny; j++)
                temp += normalized[j * input.nx + i] * x[j];

            ATx[i] = temp;
        }

        std::vector<double> AATx(input.ny);
        for (int j = 0; j < input.ny; j++) {
            double temp = 0.0;
            for (int i = 0; i < input.nx; i++)
                temp += normalized[j * input.nx + i] * ATx[i];

            AATx[j] = temp;
        }

        std::vector<double> Bx(input.ny);
        for (int j = 0; j < input.ny; j++) {
            double temp = 0.0;
            for (int i = 0; i < input.ny; i++)
                temp += result[std::min(i, j) * input.ny + std::max(i, j)] * x[i];
